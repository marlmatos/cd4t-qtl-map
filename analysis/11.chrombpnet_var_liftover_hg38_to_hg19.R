#!/usr/bin/env Rscript
.libPaths(c("~/R/x86_64-pc-linux-gnu-library/4.4","/path/to/software/easybuild/software/R/4.4.1-gfbf-2023b/lib/R/library"))
...
in_path  <- "~/cd4_chrombpnet/chrombpnet_model_b7/variant_prediction_scores/averaged_scores/average_cd4_tcells_AJ_common_variants.mean.variant_scores_significan.tsv"

# dbSNP beds (same style/locations as your SuSiE script)
dbsnp38_dir <- script11$dbsnp38_dir
dbsnp37_dir <- script11$dbsnp37_dir

# Where to drop intermediates + outputs  (use absolute path; expand ~)
base_out <- path.expand("~/cd4_qtl_paper_figures/paper_reviews/reviews_round1/data/liftover_cbpnet_rsID")
...
bedtools <- Sys.getenv("BEDTOOLS", unset="~/.conda/envs/liftover/bin/bedtools")
