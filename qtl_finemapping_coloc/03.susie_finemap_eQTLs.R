...
eqtl_path = "/gchm/cd4_QTL_analysis/04_Run_interaction_eQTLs/interaction_TF_disrupted_chromBPNet/results/interaction_chrombpnet_tfs/TF_activity_interaction_chrombpnet_tfs/ATF1/ATF1_chr4.cis_qtl_pairs.chr4.parquet"
ld_regions ="regions/ATF1.chr4.regions.txt"
...
if(grepl("parquet",eqtl_path)){
  ...
  colnames(eqtl)[colnames(eqtl) == "slope"] = "beta"
  colnames(eqtl)[colnames(eqtl) == "b_gi"] = "beta"
  colnames(eqtl)[colnames(eqtl) == "b_gi_se"] = "slope_se"
} else{
...
load_LD = function(CHR,LOWER,UPPER,ld_path,sumstats){
  ...
  system(paste0("/path/to/software/plink/plink-1.90b6.24/plink --bfile ",file_path,"_imputed --keep-allele-order --r square --out ",file_path,"_imputed"))
...
run_susie_finemap = function(x){
  ...
  for (name in genes){
    ...
    colnames(pr)[1:10] = c(paste0("lbf_", 1:10))
    ...
    for (i in seq_along(S2$sets$cs_index)) {
      cs = as.data.frame(cbind(S2$pip[S2$sets$cs[[i]]], t(S2$lbf_variable[, S2$sets$cs[[i]]]))) %>%
        mutate(variant_id = D2$snp[S2$sets$cs[[i]]],
               ...
               nsnps = length(D2$snp)) %>%
        left_join(snp_region_gene[,c("variant_id","variant_id.ss")], "variant_id") # rejoin unflipped variant ids
      colnames(cs)[1:11] = c("pip", paste0("lbf_cs", 1:10))
...
results = lapply(1:2, function(x) run_susie_finemap(x))
...
lbf_res_combined <- do.call(rbind, lapply(results, safe_extract, i = 1)) %>%
  mutate(unique_id = paste0(variant_id.ss, gene)) %>%
