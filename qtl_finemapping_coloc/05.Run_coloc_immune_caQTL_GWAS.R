...
# gwas_id = "2023_37156999_IBD_EUR-EAS_preprocessed"
...
run_coloc = function(region){
  ...
  eqtl_lbf = fread(paste0("SuSiE_finemap_lbf_GRC37/",eqtl_name,"/",eqtl_name,"_chr",chr,"_lbfs_GRC37.txt")) %>%
    ...
    filter(variant_id_GRC37 %in% gwas_lbf$variant_id_GRC37 | variant_id_GRC37_alt %in% gwas_lbf$variant_id_GRC37)
  ...
  for (mol_id in peaks){
    ...
    # Format eQTL lbf df
    eqtl_lbf2 = eqtl_lbf[peak == mol_id,] %>%
    ...
    idx_eqtl = cs_df %>% 
      filter(region %in% eqtl_lbf2$region & peak %in% eqtl_lbf2$peak) %>%
      distinct(cs) %>% unlist()
    ...
    coloc = coloc %>% dplyr::filter(PP.H4.abf > 0.5) %>% 
      dplyr::rename(GWAS_variant_GRC37 = hit1, eQTL_variant_GRC37 = hit2)
    
    cat(mol_id,"Wooo Significant colocalisation! \n")
    
    temp_df = gwas_lbf3 %>% dplyr::rename(GWAS_variant_GRC37 = variant_id_GRC37)
    temp_df2 = eqtl_lbf2 %>% dplyr::rename(eQTL_variant_GRC37 = variant_id_GRC37)
    
    coloc = coloc %>%
      left_join(temp_df, "GWAS_variant_GRC37") %>%
      left_join(temp_df2,"eQTL_variant_GRC37") %>%
      dplyr::rename(region_GRC38 = region) %>%
...
if(nrow(results2) > 0){
  ...
  fwrite(results2, paste0("coloc_results/",eqtl_name,"_",gwas_id, "_coloc_results.csv"), quote = F, row.names = F, sep = "\t")
}
