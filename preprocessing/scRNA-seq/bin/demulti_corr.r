...
# Define your script as a function (optional)
process_vcf <- function(reference_vcf, cluster_vcf, outdir) {
    ### Required so that libraries are loaded from the image instead of locally
...
if (length(args) != 3) {
    stop("Usage: Rscript script.R reference_vcf cluster_vcf outdir")
}

# Assign arguments to variables
reference_vcf <- args[1]
cluster_vcf <- args[2]
outdir <- args[3]

# Call the function with command-line arguments
process_vcf(reference_vcf, cluster_vcf, outdir)                     
