...
# ============================================================
.libPaths(c(
  "/path/to/home/R/x86_64-pc-linux-gnu-library/4.4",
  "/path/to/software/easybuild/software/R/4.4.1-gfbf-2023b/lib/R/library"
))
...
# ============================================================
paths <- list(
  atrr        = "/path/to/home/cd4_chrombpnet/chrombpnet_model_b7/variant_contribution_scores/averaged_cd4_tcells_AJ_common_variants.shap.counts.h5",
  data_h5     = "/path/to/home/cd4_chrombpnet/chrombpnet_model_b7/variant_prediction_scores/averaged_variant_prediction_scores.h5",
  tsv         = "/path/to/home/cd4_chrombpnet/chrombpnet_model_b7/variant_prediction_scores/averaged_scores/average_cd4_tcells_AJ_common_variants.mean.variant_scores.tsv",
  chrom_sizes = "/path/to/home/resources/genome/hg38.chrom.sizes",
  bw_path     = "/path/to/project/cd4_aging_project/data/ATAC-seq/atac_preprocessing/nextflow_tmp/a4/4c07c68cd77cef99a8b320e8e66238/cd4_atac.bigWig",
  peaks_bed   = "/path/to/project/cd4_aging_project/data/ATAC-seq/atac_preprocessing/merged_library/peak_calling/MACS3/BAMPE/peaks_102024/cd4_atac_padded_summits_peaks.bed",
  filtered_peaks="/path/to/home/cd4_caQTL_analysis/variant_to_peak_QTL/run_012625_qc_aware_qsmooth_CPM_MAF5_FDR5_1MB/results/001_peaks/peak_names.txt",
  ...
  summary_df  = "/path/to/home/cd4_qtl_paper_figures/figure_1/data/eqtl_caqtl_finemapping_coloc_all.tsv",
  coloc_csv   = "/path/to/lab_group/sghatan/marlis_pj/coloc/coloc_results/ca_eqtl_coloc/chr7_coloc_results.csv",
  metadata = "/path/to/home/ATAC-seq_analysis/diff_accesibility_ana/results/metadata/cd4_atac_metadata_scrna_union.csv",
  dir = '/path/to/home/cd4_caQTL_analysis/variant_to_peak_QTL/run_012625_qc_aware_qsmooth_CPM_MAF5_FDR5_1MB/results/004_genotypes/plink/per_chr_plink_files/chr',
  bigwig_dir = "/path/to/project/cd4_aging_project/data/ATAC-seq/atac_preprocessing/STAR/bigwig"
...
z_to_positive <- function(z) {
  zz <- as.numeric(scale(z))
  zz - min(zz, na.rm = TRUE)
}
...
plot_qtl_scatter <- function(markers_df, minStart, maxEnd, label_txt, region, ylab_expr = bquote(-log[10](p))) {
  maxlimit <- (ceiling(max((-1) * markers_df$z_pos0, na.rm = TRUE) / 5) * 10) + 5
  ...
  ggplot() +
    ...
    annotate(
      "text",
      x = minStart,
      y = max(markers_df$z_pos0, na.rm = TRUE),
      label = label_txt,
      hjust = 0, vjust = 1,
      size = 4
    ) +
...
#   select(
#     var_id, finemapped_cs_coloc, gene, peak, variant_id.x, variant_id.y, PP.H4.abf, coloc_status,
#     caQTL_variant, eQTL_variant, chr, pos, allele1, allele2, logfc.mean, jsd.mean, var_chr_pos,
#     is_qtl, category, peak_annotation, seqnames, variant_loc, motifs_collapsed
#   )
...
# Coloc results once
coloc_results <- fread(paths$coloc_csv)
...
region_zoom2_5      <- paste0(var_contig, ":", max(1, var_pos - 500), "-", var_pos + region_window_broad2_5)
...
prep_ca   <- prepare_caqtl_data(var_contig, peak_ca, region_ca, variant)
...
# ============================================================
atac_sub <- import.bw(paths$bw_path, which = region_zoom2_gr)
...
p.peak_delim <- trackplot_peak_delim(bed_peaks_gr, region_zoom2, "aquamarine4") +
  highlight_region(region_zoom, color = "skyblue", alpha = 0.2) +
  theme(legend.position = "none", plot.title = element_text(size = 8, hjust = 0.5))

track_genes <- trackplot_gene_custom(transcripts, str_to_gr(region_zoom2)) +
  guides(color = "none") +
  highlight_region(region_zoom, color = "skyblue", alpha = 0.2) +
  theme(legend.position = "none", plot.title = element_text(size = 8, hjust = 0.5))
...
p.peaks <- plotGroupedCoverage_allelic_fill(
  ...
  rasterize = TRUE, raster_dpi = 400)

p.peak_delim2_5 <- trackplot_peak_delim(bed_peaks_gr, region_zoom2_5, "aquamarine4") 
...
track_plot <- trackplot_combine2(
  list(
    BPCells:::wrap_trackplot(p.caqtl,       unit(0.25, "null")) + indiv_theme,
    BPCells:::wrap_trackplot(p.eqtl,        unit(0.25, "null")) + indiv_theme,
...
# ============================================================
ggsave(
  filename = "~/cd4_qtl_paper_figures/figure_2_chrombpnet/plotting/cbpnet_examples/plots/TRBV28_coloc_qtl_chrom_dyna2.pdf",
  plot     = pt,
...
pseudocells=("/path/to/home/cd4_CellRegMap/001_preprocessing/results_01312025/prep_phenotype_vector/Data/pseudobulk_metacells/Pseudobulk_per_donor_all_cells_all_conditions_Leiden_res34.h5")
mofa = "/path/to/home/cd4_CellRegMap/001_preprocessing/results_01312025/mofa_factors/mofa_trained/cd4_aging_filt_sce_mofa_expectations_model_factors_res34.csv"
...
pt3<-FeaturePlot(sce.seurat, reduction = "MOFA", dims = c(3, 1), features = c("MX1"), pt.size = 1)
pt4<-FeaturePlot(sce.seurat, reduction = "MOFA", dims = c(3, 1), features = c("OAS1"), pt.size = 1)
...
ggsave("~/cd4_qtl_paper_figures/figure_2_chrombpnet/plotting/cbpnet_examples/plots/TRBV28_dynamic_qtl.pdf", g, width = 10, height = 4, units = "in")
