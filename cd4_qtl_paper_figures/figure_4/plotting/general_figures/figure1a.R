.libPaths(c("/gchm/R/x86_64-pc-linux-gnu-library/4.4","/path/to/software/easybuild/software/R/4.4.1-gfbf-2023b/lib/R/library"))
...
cat("Reading data") 
out.dir="/gchm/cd4_qtl_paper_figures/figure_1/plotting/plots/"
...
#summary_df<-read_delim("/path/to/project/cd4_aging_project/cd4_qtl_data/colocalization/caqtl_eqtl_coloc_finemapped/eqtl_caqtl_finemap_coloc_summary_chr19.tsv")
...
cs_counts <- summary_df %>%
  group_by(coloc_status) %>%
  summarise(
    n_cs = n_distinct(finemapped_cs_coloc),
    .groups = "drop"
  )

caQTL_counts <- summary_df %>%
  group_by(finemapped_cs_caqtl) %>%
  summarise(
    n_cs = n(),
    .groups = "drop"
  )
...
cBPNet_var_only<-cBPNet_var %>% filter(!(variant_id %in% chrombpet_shared_vars)) %>% select(variant_id) %>% #unique the chrombpnet variants
  mutate(coloc_status=NA,
         cs_type_any_var=NA)

unique_coloc_vars <- unique(unlist(c(chrombpnet_caqtlvars, chrombpnet_eqtlvars))) #all credible sets that contain a chromBPnet var (union from the frist two steps)

print(paste0("The number of chromBPnet variants that are also QTLs is: " ,print(length(unique_coloc_vars)))) 

unique(summary_df$coloc_status)

credible_sets <- summary_df %>% 
  distinct(finemapped_cs_coloc, coloc_status, cs_type_any_var) 
colnames(credible_sets)<-c("variant_id",  "coloc_status", "cs_type_any_var")

credible_sets <- credible_sets %>% rbind(cBPNet_var_only) %>% 
  mutate(caQTL_category = cs_type_any_var,
         chromBPNet = variant_id %in% unique_coloc_vars) %>%
...
saveRDS(credible_sets, "~/cd4_qtl_paper_figures/figure_1/data/upset_credible_sets.rds")
...
levels_inpeak <- c("Within caQTL peak",
                   "In peak correlated with caPeak",
                   "In other open peak")
...
all_vars <- all_vars %>%
  mutate(variant_id.plink = to_b38_variant_id(var_id_std))

#read plink calculated MAFs
mafs<-fread("/gchm/cd4_QTL_analysis/01_genotype_snps_covar/02_genotype_covariates/analysis/002.v4_calculating_pcs_MAF5/CD4_all_chr_ashkenazi.363.AF1.QC.BA.king2.hwe.scrnaseq.annot.freq.frq")
...
barplot_stacked <- all_vars %>% 
  filter(!is.na(MAF)) %>%
  mutate(
    MAF_bin = cut(MAF, breaks = quantile(MAF, probs = seq(0,1,0.2), na.rm = TRUE),
  ...
  geom_bar(position = "fill") +
  scale_fill_carto_d(palette = "Magenta", name = "MAF Bin") +
  labs(
    x = "Group",
    y = "Prop. Variants with MAF bins"
  ) +
...
upset_sharing4 <- barplot_stacked / barplot_tss / prop_plot / up_base +
  plot_layout(
    heights = c(1, 1, 1, 4),
    guides  = "collect"
  )
...
cs_annot <- all_vars_annot %>%
  ...
  summarise(
    anno_cs = case_when(
      ...
      TRUE ~ NA_character_                           # keep NA if no annotated variants
    ),
    .groups = "drop"
  ) %>%
  mutate(
    anno_cs = forcats::fct_na_value_to_level(factor(anno_cs,
                                              levels = c("promoter", "distal")),
                                       na_level = "NA")
  )
...
cre_annot_barplot<-all_vars_annot %>%
  group_by(intersection, anno_cs.y) %>%
  summarise(count = n(), .groups = "drop") %>%
  ggplot(aes(x = intersection, y = count, fill = anno_cs.y)) +
...
upset_sharing4 <- barplot_stacked /cre_annot_barplot/ barplot_tss / prop_plot / up_base +
  plot_layout(
    heights = c(1, 1, 1, 1, 4),
    guides  = "collect"
  )
...
#   group_by(cs_type_any_var) %>%
#   summarise(n = n_distinct(finemapped_cs)) %>%
#   mutate(unit = "caQTLs") %>% 
#   ggplot(aes(x = unit, y = n, fill = cs_type_any_var)) +
...
# names(colors) <- c("in_caPeak", "in_corr_Peak", "in_other_Peak", "no_Peak_overlap")
# 
# var_distance<-ggplot(binned_norm_matched, aes(x = bin_center, y = norm_peak_count, color = cs_type_any_var)) +
